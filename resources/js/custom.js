...
$(function() {
     ...
     * show confirm dialog
     */
    function showConfirmDialog(text, confirmButtonText = 'OK', cancelButtonText = 'キャンセル') {
        return Swal.fire({
            text: text,
            icon: 'question',
    ...
    _common.showConfirmDialog = showConfirmDialog;
});
...
$(document).ready(function() {
    ...
    $('.btn-clear-search').click(function() {
        ...
        closestForm.find('.chosen-select').val('').trigger('chosen:updated');
    ...
    setTimeout(function() {
        if (changeTrackingForm.length > 0) {
            ...
            $('body').delegate(
                'a:not(".dropdown-toggle, .not-tracking"), button:not(".btn-submit, .not-tracking")',
                'click',
                backConfirmation
            );

            $('body').on('.btn-submit', 'click', function () {
                // window.SAFE_LEAVE = true;
            });
            ...
            $(window).on('beforeunload', function() {
                if (
                    !window.SAFE_LEAVE &&
                    changeTrackingForm.data('originalValue') !== changeTrackingForm.serialize()
                ) {
