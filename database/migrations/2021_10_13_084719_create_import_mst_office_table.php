...
class CreateImportMstOfficeTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('import_mst_office', function (Blueprint $table) {
            $table->unsignedBigInteger('id')->autoIncrement();
            $table->string('valid_date', 255)->nullable();
            for ($x = 1; $x <= 12; $x++) {
                if ($x <= 9) {
                    $table->text('destination_0'.$x)->nullable();
                } else {
                    $table->text('destination_'.$x)->nullable();
                }
            }
            for ($x = 1; $x <= 24; $x++) {
                if ($x <= 9) {
...
//            }
    ...
    {
        Schema::dropIfExists('import_mst_office');
    }
}
